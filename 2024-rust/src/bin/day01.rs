...
fn solve_for(input: &str) -> Result<String> {
    ...
    let list_2_occurrences = list_2.iter().fold(FxHashMap::<u32, u32>::default(), |mut b, &n| {  *(b.entry(n).or_default()) += 1; b } );

    let list_1_similarities = list_1.iter().map(|n| n * list_2_occurrences.get(n).unwrap_or(&0)).sum::<u32>();
