...
fn solve_for(input: &str) -> (u64, u64) {
    ...
    // dbg!(&inputs, &gates);
    //
    let values = inputs.keys().map(|k| (*k, false));
    let mut values = FxHashMap::from_iter(values);
    for (wire, value) in inputs {
        values.entry(wire).insert_entry(value);
    }
    ...
    'outer: loop {
        ...
        for (wire, gate) in &gates {
            ...
            if !values.contains_key(wire) && values.contains_key(l) && values.contains_key(r) {
                ...
                eprintln!("{} ({}) {} {} ({}) -> {} ({})", l, l_value, op, r, r_value, wire, new_value);

                values.entry(wire).insert_entry(new_value);
    ...
    let z_values = values.iter().filter(|(k, v)| k.starts_with('z')).sorted().map(|(k, v)| v);
    // dbg!(&z_values);

    let mut part1 = 0;
    let mut p2 = 0;
    for z in z_values {
        let z = if *z { 1_u64 } else { 0 };
