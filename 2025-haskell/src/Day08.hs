...
instance Hashable Connection where
  hashWithSalt _ a = hash (cdist a) + hash (b1 a) + hash (b2 a)
...
connect ba bb = Connection (round dist) ba bb
  where
    dist :: Double
...
allConnections bs = concatMap (\ba -> map (connect ba) $ filter (/= ba) bs) bs
...
canConnect cs bs = find (\c -> HashSet.member (b1 c) bs || HashSet.member (b2 c) bs) (HashSet.toList cs)
...
getTrees (p, c, d)
  ...
  | null c =
      let next = fromJust . listToMaybe $ HashSet.toList p
...
part1 count input = result
  where
    ...
    top3 = take 3 $ sortBy (comparing Data.Ord.Down) sizes
    result = product top3

getTreeSizes :: [Connection] -> [Int]
getTreeSizes cs =
  let seed = (HashSet.fromList cs, HashSet.empty, [])
      (_, _, sizes) = run getTrees seed
   in sizes
...
part2 input = result
  where
    ...
    (aconns, _) = fromJust . listToMaybe $ complete
    aconn = last aconns
